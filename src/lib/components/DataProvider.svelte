<script lang="ts">
	...
	import { PUBLIC_CACHE_TTL, PUBLIC_ARROW_URL, PUBLIC_CURRENCIES_URL } from '$env/static/public';
	import { initDuckDB } from '$lib/db/duckdb';
	import { validateSchema } from '$lib/db/schema';
	...
	import { DataCacheService } from '$lib/db/cache';
	...
	let cacheService: DataCacheService | null = $state(null);
	...
	async function loadData(forceRefresh: boolean = false) {
		...
		try {
			...
			if (!cacheService) {
				cacheService = new DataCacheService({ ttl: cacheTtl });
